	...
	<div class="container">
		<div class="topic" style="margin-top: 50px;"><h1><?php the_title(); ?></h1></div>
			<div class="row">
				...
				Метки:	<?php the_tags( $before, $separator, $after ); ?>
				</div>
			</div>	
		</div> <!-- container off -->
...
if( get_adjacent_post(false, '', true) ) { 
  previous_post_link('%link', '<i class="fas fa-long-arrow-alt-left"></i> Предыдущий пост' ); 
	...
	$first = new WP_Query('posts_per_page=1&order=DESC&post_type=blog');
	$first->the_post();
...
if( get_adjacent_post(false, '', false) ) { 
	...
	$last = new WP_Query('posts_per_page=1&order=ASC&post_type=blog');
	$last->the_post();
